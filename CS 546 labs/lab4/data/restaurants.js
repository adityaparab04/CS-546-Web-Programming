...
const create = async function create(name, location, phoneNumber, website, priceRange, cuisines, serviceOptions){    
    ...
    if (arguments.length !== 7) throw new Error ("Enter all the properties of a restaurant");
...
// update function
const update = async function update(id, name, location, phoneNumber, website, priceRange, cuisines, overallRating, serviceOptions){
    ...
    if(!id || typeof id !== 'string' || !id.replace(/\s/g, "").length){
        throw new Error ("Please enter valid inout id as a string")
    }
